...
use LavaMusic\ElasticSearch\Common\Exceptions\BadRequest400Exception;
use LavaMusic\ElasticSearch\Common\Exceptions\Conflict409Exception;
use LavaMusic\ElasticSearch\Common\Exceptions\ElasticsearchException;
use LavaMusic\ElasticSearch\Common\Exceptions\Forbidden403Exception;
...
use LavaMusic\ElasticSearch\Common\Exceptions\NoDocumentsToGetException;
use LavaMusic\ElasticSearch\Common\Exceptions\NoShardAvailableException;
use LavaMusic\ElasticSearch\Common\Exceptions\RequestTimeout408Exception;
use LavaMusic\ElasticSearch\Common\Exceptions\RoutingMissingException;
use LavaMusic\ElasticSearch\Common\Exceptions\ScriptLangNotSupportedException;
use LavaMusic\ElasticSearch\Common\Exceptions\ServerErrorResponseException;
use LavaMusic\ElasticSearch\Common\Exceptions\TransportException;
use LavaMusic\ElasticSearch\Common\Exceptions\Unauthorized401Exception;
...
class Connection implements ConnectionInterface
{
    ...
    )
    {
        ...
        if (isset($connectionParams['client']['headers']['Authorization']) === false
            && isset($connectionParams['client']['swoole']['http_auth']) === false
            && isset($hostDetails['user'])
            && isset($hostDetails['pass'])
        ) {
            $connectionParams['client']['swoole']['http_auth'] = 'base_auth';
            $connectionParams['client']['swoole']['user_pwd_str'] = $hostDetails['user'] . ':' . $hostDetails['pass'];
        }

        $connectionParams['client']['swoole']['port'] = $hostDetails['port'];
    ...
    public function performRequest(string $method, string $uri, ?array $params = [], $body = null, array $options = [], Transport $transport = null)
    {
        ...
        // the param client is unnecessary if client is empty
    ...
    private function wrapHandler(callable $handler): callable
    {
        return function (array $request, Connection $connection, Transport $transport, $options) use ($handler) {
            ...
            $responseHandler = function ($response) use ($connection, $transport, $request, $options) {
                $this->lastRequest['response'] = $response;

                if (isset($response['error']) === true) {
                    if ($response['error'] instanceof ConnectException) {
                        $this->log->warning("Swoole http client request exception encountered.");
    ...
    {
        $port = $request['client']['swoole']['port'] ?? $response['transfer_stats']['primary_port'] ?? '';
    ...
    {
        return $this->connectionParams['client']['swoole']['user_pwd_str'] ?? null;
    }
    ...
    protected function getCurlRetryException(array $request, array $response): ElasticsearchException
    {
        ...
        return new SwooleRequestException($response['swoole']['errno'], $message, $exception);
    }
